  ...
  await (async function $for_i_0_i_10_i$1() {
   ...
   await (async function $for_i_0_i_10_i$1() {
    ...
    function asyncSpawn(ast) {
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            descend();
            if ((node.type==='ArrowFunctionExpression' || examine(node).isScope) && containsAwait(node.body)) {
                blockifyArrows(node);
                ...
                mapLogicalOp(node);
                mapCondOp(node);
                walkDown(node, [mapBlock,mapTryCatch,mapIfStmt,mapSwitch]);
    ...
    function getDeclNames(id) {
        ...
        case 'ArrayPattern':
            ...
            break ;
        case 'ObjectPattern':
            return id.properties.map(function(p){ return p.key.name }) ;
            break ;
        case 'RestElement':
    ...
    function checkConstsNotAssigned(ast) {
        ...
        function fail(node){
            ...
            /*var ex = new SyntaxError(where(node) + "attempt to assign to const "+printNode(node), pr.filename, node.start);
            ex.pos = node.start;
            ex.loc = node.loc.start;
            throw ex;*/
        }
    ...
    function hoistDeclarations(ast) {
        ...
        function insideIndex(node,path) {
            for (var i=0; i<path.length; i++) {
                if (path[i].parent===node)
                    return path[i].index ;
            }
        }
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (examine(node).isBlockStatement) {
                ...
                if (await) {
/*                    var awaitIdx = insideIndex(node,await) ;
                    function beforeAwait(c){
                        return awaitIdx <= insideIndex(node,c) ;
                    }*/
...
//                        consts = consts.filter(beforeAwait) ;
                        ...
                        // only valid in node 6 or in strict mode on node 4/5, which can be implemented via 'let'
